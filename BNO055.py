...
#       GPIO Pins & LEDs: lines 17 -> 30
#       flash() at lines 221 -> 228
#       connect_IMU(self) at lines 428-> 446
#       position(self) at lines 733 -> 743
#       calibrate(self) at lines 744 -> 765
...
import pdb
import serial
import Adafruit_GPIO as GPIO    # Note: Using Adafruit GPIO setup instead of RPI

# Note: Below assumes BCM Pin Numbering ie not board numbering
gpio = GPIO.get_platform_gpio()

# Setup LEDs for Calibration
#GPIO.setwarnings(False)
#GPIO.setmode(GPIO.BOARD)
LED0=16; LED1=20; LED2=21; LED3=26
gpio.setup(LED0,GPIO.OUT)
gpio.setup(LED1,GPIO.OUT)
gpio.setup(LED2,GPIO.OUT)
gpio.setup(LED3,GPIO.OUT)
gpio.output(LED0,0); gpio.output(LED1,0); gpio.output(LED2,0); gpio.output(LED3,0) # Lights OFF
...
def lights_on():
        gpio.output(LED0,1); gpio.output(LED1,1); gpio.output(LED2,1); gpio.output(LED3,1) # Lights ON
        time.sleep(0.2)
...
class BNO055(object):
    ...
    def start(self, mode=OPERATION_MODE_NDOF):
    ...
    def connect_IMU(self):  #While loop in thread to skip incriments where IMU does not successfully initiate.
        while True:
            try:
                # Initialize the BNO055 and stop if something wrong.
                if not self.start():
    ...
    def position(self):
        ...
        data=[heading, roll, pitch,sys, gyro, accel, mag]
    ...
    # This will be for the calibration step (with LEDs to indicate status)
    def calibrate(self):
        names=['Gyroscope','Accelerometer','Magnometer','System']
        for i in range(0,4):
            lights_off()    
            cali_list = [0,0,0,0]
            while cali_list[i]!=3:
                sys, gyro, accel, mag = self.get_calibration_status() 
                cali_list = [gyro, accel, mag, sys]
                print('{0} calibration at {1}'.format(names[i],cali_list[i]))
                if cali_list[i]==0:
                    gpio.output(LED0,1); gpio.output(LED1,0); gpio.output(LED2,0)
                elif cali_list[i]==1:
                    gpio.output(LED0,1); gpio.output(LED1,1); gpio.output(LED2,0)
                elif cali_list[i]==2:
                    gpio.output(LED0,1); gpio.output(LED1,1); gpio.output(LED2,1)
                time.sleep(0.5)
            lights_off()
            flash(i)
            print('{0} Ok'.format(names[i]))
            time.sleep(2.5)
        lights_off()
...
# Make connection between Pi & IMU (avoiding errors)
bno.connect_IMU()
